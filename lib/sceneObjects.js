...
function BarChart(A, B)
{
	...
	this.B					= B;
	this.values				= [0,0,0,0];
	this.userValues			= [undefined, undefined, undefined, undefined];
	this.valuesFunctionStr	= "return 10-index+1;";
	this.showMip			= 1;
	this.visible			= true;
	this.frozen				= false;
	this.width				= 1;
	...
	this.onChange = function()
	{
		...
		for (var i = 0; i != this.values.length; ++i)
		{
			var p0 = mad(tan, this.values[i], mad(unit, i*delta, this.A));
			var p1 = mad(tan, this.values[i], mad(unit, (i+1)*delta, this.A));
	...
	this.draw = function(camera)
	{
		...
		if (this.showMip>0)
		{
			var count = this.values.length / Math.pow(2, this.showMip);

			var delta = distance(this.A, this.B) / count;

			count = Math.floor(count);

			var mip0count = this.values.length / count;
			mip0count = Math.floor(mip0count);
			...
			for (var i=0; i!=count; ++i)
			{
				...
				for (var j=0; j!=mip0count; ++j)
				{
					...
					//if ( (mip0count%1) && (j==0 && odd) )
					//{
					//	avgVal += value * (mip0count%1);
					//}
					//else if ( (mip0count%1) && (j==mip0count-1 && !odd) )
					//{
					//	avgVal += value * (1 - (mip0count%1));
					//}
					//else
					{
						avgVal += value;
				...
				camera.drawLineStrip([min1, max1, max2, min2, min1], this.color, width, [], "rgba(0,0,0,0.5)", this);
				//camera.drawLine(avg1, avg2, this.color, width, [], undefined, this);
			}
		}
...
function ScreenshotArea(p, okCallback, cancelCallback)
{
	...
	this.draw = function(camera)
	{
		...
		var a = new Vector(pmin.x, pmax.y);
		var b = new Vector(pmax.x, pmin.y);

		camera.drawRectangle(new Vector(l,t), new Vector(a.x, a.y), "rgba(0,0,0,0.5)", 1, [], "rgba(0,0,0,0.5)");
		camera.drawRectangle(new Vector(a.x,t), new Vector(b.x, a.y), "rgba(0,0,0,0.5)", 1, [], "rgba(0,0,0,0.5)");
		camera.drawRectangle(new Vector(b.x,t), new Vector(r, a.y), "rgba(0,0,0,0.5)", 1, [], "rgba(0,0,0,0.5)");

		camera.drawRectangle(new Vector(l,a.y), new Vector(a.x, b.y), "rgba(0,0,0,0.5)", 1, [], "rgba(0,0,0,0.5)");
		camera.drawRectangle(new Vector(b.x,a.y), new Vector(r, b.y), "rgba(0,0,0,0.5)", 1, [], "rgba(0,0,0,0.5)");

		camera.drawRectangle(new Vector(l,b), new Vector(a.x, b.y), "rgba(0,0,0,0.5)", 1, [], "rgba(0,0,0,0.5)");
		camera.drawRectangle(new Vector(a.x,b), new Vector(b.x, b.y), "rgba(0,0,0,0.5)", 1, [], "rgba(0,0,0,0.5)");
		camera.drawRectangle(new Vector(b.x,b), new Vector(r, b.y), "rgba(0,0,0,0.5)", 1, [], "rgba(0,0,0,0.5)");
	...
	this.getProperties = function()
	{
		return	[
					...
					{ name: "cm per Unit", control: new Slider(0.5, 4, this.CMperUnit, 0.5, function (value) { this.CMperUnit = value; }.bind(this)) },
