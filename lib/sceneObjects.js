...
function FunctionGraph(origin)
{
	...
	this.precalculateValues = function()
	{
		...
		if (this.doConvolution)
		{
			...
			var dxMin = (this.coordinateType == 0) ? (this.xMin-this.xMax) : (-Math.PI);
			var dxMax = (this.coordinateType == 0) ? (this.xMax-this.xMin) : (+Math.PI);
			var count = Math.ceil((dxMax - dxMin) / evalStep) + 1;

			this.convolutionFunctionValues = new Array(count);

			var index = 0;
			for (var dx = dxMin; dx <= dxMax; dx += evalStep)
			{
	...
	this.graphFunction = function(x, useCache)
	{
		...
		if (this.doConvolution)
		{
			...
			for (var dx = dxMin; dx <= dxMax; dx += evalStep)
			{
				...
				if (useCache)
				{
					var index = Math.ceil((dx - dxMin) / evalStep);
				...
				if (useCache)
				{
					var index = Math.ceil((xx - this.xMin) / evalStep);
