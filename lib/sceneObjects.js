...
function ArcWall(centerPos, radius, startAngle, endAngle)
{
	...
	this.rayHitTest = function(rayPos, rayDir, rayRadius)
	{
		...
		// on the ray and the center is equal to arc radius + ray radius?
		var collisionRadius = this.convex ? (this.radius + rayRadius) : (this.radius - rayRadius);
		var delta = Math.sqrt(collisionRadius*collisionRadius - perpDist*perpDist);
...
function BouncingBall(position)
{
	...
	this.onFrameTick = function(deltaTime_ms)
	{
		...
		if (this.currentVelocity.lengthSqr()>0)
		{
			...
			if (this.impactPoint.hit && this.impactPoint.tRay <= length(positionDelta))
			{
	...
	this.draw = function(camera)
	{
		...
		//if (this.impactPoint != undefined && this.impactPoint.hit)
		//{
	...
	this.updateDragVelocity = function()
	{
		if (this.externalDragActive)
		{
			...
			else
			{
				...
				var decay = 0.1;
				this.externalDragVelocity = add(mul(this.externalDragVelocity, decay), div( deltaPos, deltaTime / 1000));
	...
	this.getProperties = function()
	{
		...
		return	[
					...
					{ name: "Density", control: new PopoutSlider(0, 5, this.density, 0.1, function (value) { this.density = value; this.onChange(); }.bind(this)) },
