...
function Wall(_points)
{
	...
	this.draw = function(camera)
	{
		...
		var width = (this.selected ? 4 : 1) * this.appearance.lineWidth;
		...
		camera.drawLines(linePoints, this.appearance.GetLineColor(), width, this.appearance.GetLineDash(), undefined, this);
		camera.drawLines(hatchingPoints, this.appearance.GetLineColor(), width, [], undefined);
	}
	...
	{
		var appearanceControl = new PopoutContainer(undefined, "images/distribute_horizontal.svg");
		{
...
function BRDFRay(O, dir)
{
	...
	this.draw = function(camera)
	{
		...
		var rays = [{ start: this.O, dir: this.dir, level:0, color:this.color, intensity:this.intensity }];
		...
		camera.drawArrow(this.O, add(this.O, this.dir), 20, "#000000", width);
	}
...
function PointLight(O, radius)
{
	...
	this.draw = function(camera)
	{
		...
		camera.drawStar(this.center, 7, this.bulbRadius, 0.5, 0, this.color, width, "rgba(255,255,0,0.8)");
		camera.drawArc(this.center, this.bulbRadius*0.5, 0, Math.PI*2, this.color, width, "rgba(255,255,0,1)");
		...
		if (this.collisionOutline)
		{
			...
			for (var a=0; a<=Math.PI*2; a+=angleStep)
			{
				...
				for (var o=0; o<this.scene.objects.length; ++o)
				{
					...
					var hits = this.scene.objects[o].rayHitTest(this.center, dir);
		...
		{
			camera.drawArc(this.center, this.radius, 0, Math.PI * 2, this.color, 1, "rgba(0,0,0,0)");
		}
	}
	...
	this.draw = function(camera)
	{
		...
		// Draw arrows
		{
			...
			for (var x=0; x<=stepCount; ++x)
			{
				...
				camera.drawArrow(point, mad(this.dir, 2, point), 20, this.color, 2);
			}

			camera.drawArrow(this.O, mad(this.dir, 3, this.O), 20, this.color, 2);
		}
	...
	this.draw = function(camera)
	{
		...
		if (this.spanCount>1)
		{
			...
			for (var i=1; i!=this.spanCount; ++i)
			{
				...
				camera.drawLine(mad(tan, spanWidth/2, p), mad(tan, -spanWidth/2, p), this.color, width, [], this);
			}
		}
		...
		// Eye
		{
			//camera.drawCross(this.apexPoint, camera.invScale(10), 0, this.color, width);
			camera.drawLine(this.apexPoint, mad(fromAngle(toAngle(this.dir) + 30 * Math.PI / 180), camera.invScale(50), this.apexPoint), this.color, width);
			camera.drawLine(this.apexPoint, mad(fromAngle(toAngle(this.dir) + -30 * Math.PI / 180), camera.invScale(50), this.apexPoint), this.color, width);
			camera.drawArc(mad(this.dir, camera.invScale(30), this.apexPoint), camera.invScale(6), toAngle(this.dir) + 95 * Math.PI / 180, toAngle(this.dir) - 95 * Math.PI / 180, this.color, width, "rgba(0,0,0,0.7)");
			camera.drawArc(this.apexPoint, camera.invScale(30), toAngle(this.dir) - 30 * Math.PI / 180, toAngle(this.dir) + 30 * Math.PI / 180, this.color, width);
		}
	}
		...
		if (this.pixelCount > 0)
		{
			...
			for (var fc=1, fx=-this.farWidth/2+farStep, nx = -this.nearWidth/2+nearStep; fc<this.pixelCount*2; ++fc, fx+=farStep, nx+=nearStep)
			{
				...
				if (fc % 2)
				{
					if (this.showCenterLines)
					{
						if (intersectRectLine(a, b, pointNear, pointFar))	return true;
					}
				}
				else
				{
...
function Line(_points)
{
	...
	this.draw = function (camera)
	{
		...
		for (var mip=0; mip<=this.pixelMip; ++mip)
		{
			...
			var fillColorRGB = hex2rgb(this.fillColor);
		...
		{
			// Trully lazy!
			camera.drawArrow(avg(this.points[0], this.points[1]), this.points[1], 20, this.color, width, dash, this);
			camera.drawArrow(avg(this.points[0], this.points[1]), this.points[0], 20, this.color, width, dash, this);
		}
		else
		{
			var points = this.points.slice();
			...
			if (this.closed)
			{
				fillColorRGB = hex2rgb(this.fillColor);
			...
			if (this.arrowEnd)
			{
				camera.drawArrow(points[points.length-2], points[points.length-1], 20, this.color, width, dash, this);
			}

			if (this.arrowStart)
			{
				camera.drawArrow(points[1], points[0], 20, this.color, width, dash, this);
			}
	...
	this.getProperties = function()
	{
		return	[
					...
					{name: "Fill Alpha", control: new PopoutSlider(0, 1, this.fillAlpha, 0.1, function (value) { this.fillAlpha = value; lastRectAlpha = this.fillAlpha; this.onChange(); }.bind(this)) },
					{name: "Width", control: new PopoutSlider(0, 5, this.width, 1, function (value) { this.width = value; this.onChange(); }.bind(this)) },
...
function BRDFHemisphere(centerPos, radius, normal)
{
	...
	this.draw = function(camera)
	{
		...
		camera.drawArrow(this.center, mad(this.normal, this.radius * 0.5, this.center), 20, "#000000", width);
...
function NGon(centerPos, radius)
{
	...
	this.draw = function(camera)
	{
		...
		{
			camera.drawArc(this.center, this.radius, 0, Math.PI*2, this.color, width, fillColorRGBA, dash);
		}
		else
		{
...
function Dimension(A, B, center)
{
	...
	this.draw = function(camera)
	{
		...
		else
		{
			...
			{
				camera.drawArrow(avg(offsetA0, offsetB0), offsetA0, 20, "#000000", width);
				camera.drawArrow(avg(offsetA0, offsetB0), offsetB0, 20, "#000000", width);
			}
			else if (this.offsetX < 0)
			{
				camera.drawArrow(lerp(offsetA0, offsetB0, this.offsetX), offsetA0, 20, "#000000", width);
				camera.drawLine(offsetA0, offsetB0, "#000000", width);
				camera.drawArrow(mad(sub(offsetB0, offsetA0).unit(), camera.invScale(30), offsetB0), offsetB0, 20, "#000000", width);

				textAlign = "left";
			}
			else if (this.offsetX > 1)
			{
				camera.drawArrow(lerp(offsetA0, offsetB0, this.offsetX), offsetB0, 20, "#000000", width);
				camera.drawLine(offsetA0, offsetB0, "#000000", width);
				camera.drawArrow(mad(sub(offsetA0, offsetB0).unit(), camera.invScale(30), offsetA0), offsetA0, 20, "#000000", width);
...
function BarChart(A, B)
{
	...
	this.hitTest = function(a, b, camera)
	{
		...
		for (var i = 0; i != this.values.length; ++i)
		{
			...
			if (this.fillAlpha > 0)
			{
				if (intersectRectOBB(a,b, [p0,p1,p2,p3]))
				{
					return true;
				}
			}
	...
	this.getProperties = function()
	{
		return	[
					...
					{name: "Values Function", control: new PopoutTextBox(this.valuesFunctionStr, false, false, 0, function (value) { this.valuesFunctionStr = value; this.onChange(); }.bind(this)) }
				];
	}
...
// Function Graph
// ----------------------------------------------------------------------------------------------------------------
var lastFunctionGraphColor = "#900090";
var lastFunctionGraphFillColor = "#000000";
var lastFunctionGraphAlpha = 0;
...
function FunctionGraph(origin)
{
	...
	this.fillAlpha			= lastNGonAlpha;
	this.width				= 2;
	this.functionStr		= "return Math.sin(x/5)*5;";
	this.onChangeListeners	= [];
	this.boundsMin			= new Vector(Number.MAX_VALUE,Number.MAX_VALUE);
	this.boundsMax			= new Vector(-Number.MAX_VALUE,-Number.MAX_VALUE);
	...
	this.saveAsJavascript = function()
	{
		...
		str += "functionGraph.color = \"" + this.color + "\";\n";
		str += "functionGraph.fillColor = \"" + this.fillColor + "\";\n";
		str += "functionGraph.fillAlpha = " + this.fillAlpha + ";\n";
		...
		str += "functionGraph.xStep = " + this.xStep + ";\n";
		...
		str += "functionGraph.width = " + this.width + ";\n";
	...
	this.autoFit = function()
	{
		...
		var yFunction = Function("x", this.functionStr);

		var xStep = Math.max(this.xStep, 0.001);
		...
		this.yLimitMin = computed_yMin;
		this.yLimitMax = computed_yMax;
	}
	...
	this.draw = function(camera)
	{
		...
		// Axis labels
		{
			...
			if (this.yLabel != 2)
			{
				var xSpacingPixels = camera.scale(xStep);
				var deltaY = (this.yLimitMax - this.yLimitMin);
				var yStepCount = Math.floor( camera.scale(deltaY) / xSpacingPixels );
				var yStep = deltaY / yStepCount;

				for (var y = this.yLimitMin; y <= this.yLimitMax; y += yStep)
				{
		...
		// Graph
		{
			...
			{
				var x0 = x;
				var x1 = Math.min(this.xMax, x + xStep);

				var y0 = yFunction(x0);
				var y1 = yFunction(x1);

				// Check if we need to do any clipping
				var oob0 = y0>this.yLimitMax || y0<this.yLimitMin;
				var oob1 = y1>this.yLimitMax || y1<this.yLimitMin;
				...
				p0 = add(p0, mul(this.xAxis, x0));
				p0 = add(p0, mul(this.yAxis, y0));

				var p1 = this.graphOrigin.copy();
				p1 = add(p1, mul(this.xAxis, x1));
				p1 = add(p1, mul(this.yAxis, y1));
				...
				//camera.drawRectangle(p0, camera.invScale(10), this.color, 2);
			}

			camera.drawLines(points, this.color, this.width, [], undefined, this);
		}
	}
	...
	this.getDragPoints = function(localSpace, camera)
	{
		...
		points.push(add(this.origin, mul(this.xAxis, this.graphOriginLocal.x + this.xMin)));
		points.push(add(this.origin, mul(this.xAxis, this.graphOriginLocal.x + this.xMax)));
		points.push(add(this.origin, mul(this.yAxis, this.graphOriginLocal.y + this.yLimitMin)));
	...
	this.setDragPointPos = function(index, p, localSpace)
	{
		...
		{
			var limit = this.xMax - (this.xMax - this.xMin) / 10;
		...
		{
			var limit = this.xMin + (this.xMax - this.xMin) / 10;
		...
		{
			var limit = this.yLimitMax - (this.yLimitMax - this.yLimitMin) / 10;
		...
		{
			var limit = this.yLimitMin + (this.yLimitMax - this.yLimitMin) / 10;
	...
	this.getProperties = function()
	{
		...
		{
			xAxis.addControl("X Step", new Slider(0, 5, this.xStep, 0.1, function (value) { this.xStep = value; this.onChange(); }.bind(this)) );
		...
		return	[
					{name: "Graph Color", control:new ColorPicker(this.color, function(value){ this.color = value; lastRectLineColor = this.color; this.onChange(); }.bind(this) )},
					{name: "Line Width", control: new PopoutSlider(0, 5, this.width, 1, function (value) { this.width = value; this.onChange(); }.bind(this)) },
					{name: "Function", control: new PopoutTextBox(this.functionStr, false, false, 0, function (value) { this.functionStr = value; this.onChange(); }.bind(this)) },
...
function Text(anchorPoint, initialText)
{
	...
	this.draw = function(camera)
	{
		...
		if (this.selected)
		{
			camera.drawLine(this.anchorPoint, mad(xAxis, this.maxWidth, this.anchorPoint), this.color, 1, [3, 3]);
		}
		...
		for (var i=0; i!=this.lines.length; ++i)
		{
			...
			camera.drawText(p, this.lines[i], this.color, "left", this.angle, font);
		}
	}
	...
	this.hitTest = function(a, b, camera)
	{
		...
		for (var i=0; i!=this.lines.length; ++i)
		{
			...
			if (intersectRectLine(a, b, mad(yAxis, vOffset, mad(xAxis, hMin, this.anchorPoint)), mad(yAxis, vOffset, mad(xAxis, hMax, this.anchorPoint))))
			{
				return true;
			}
		}
...
function PageOutline(origin)
{
	...
	this.draw = function(camera)
	{
		...
		camera.drawRectangle(this.boundsMin, this.boundsMax, "#000000", width);
		...
		if (this.margin>0)
		{
			...
			camera.drawRectangle(a, b, "#000000", width/2, [5,5]);
		}
	}
...
function Axis(firstPoint)
{
	...
	this.draw = function(camera)
	{
		...
		camera.drawLine(this.p0, this.p1, "#000000", width);
	...
	this.hitTest = function(a, b, camera)
	{
		if (intersectRectLine(a, b, this.p0, this.p1))
		{
			return true;
		}
...
function Bitmap(origin, pixelWidth, pixelHeight, pixelValues)
{
	...
	this.hitTest = function(a, b, camera)
	{
		//intersectRectOBB(a,b, [p0,p1,p2,p3]))
