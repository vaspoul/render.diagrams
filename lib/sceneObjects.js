...
function PointLight(O, radius)
{
	...
	this.draw = function(camera)
	{
		...
		if (this.collisionOutline)
		{
			...
			if (points.length>1)
			{
				camera.drawLineStrip(points,  this.color, width);
			}
		}
		else
		{
...
function Line(_points)
{
	this.scene					= null;
	this.points					= _points;
	this.pointTypes				= Array(_points.length).fill(1);	// 0: corner, 1: symmetrical, 2: asymmetrical
	this.segmentControlPoints	= Array(_points.length-1);
	this.isBezier				= false;
	this.closed					= false;
	this.selected				= false;
	this.visible				= true;
	...
	this.saveAsJavascript = function()
	{
		...
		for (var i = 0; i != this.pointTypes.length; ++i)
		{
			...
			str += this.pointTypes[i];
		}

		str += "];\n";

		str += "lineObject.segmentControlPoints = [";

		for (var i = 0; i != this.segmentControlPoints.length; ++i)
		{
			if (i > 0)
			{
				str += ", ";
			...
			if (this.segmentControlPoints[i] == null)
			{
				str += "null";
			}
			else
			{
				str += "[ new Vector(" + this.segmentControlPoints[i].c0.x + ", " + this.segmentControlPoints[i].c0.y + "), ";
				str += ", new Vector(" + this.segmentControlPoints[i].c1.x + ", " + this.segmentControlPoints[i].c1.y + ")] ";
			}
		}
	...
	this.addPoint = function(point)
	{
		...
		this.segmentControlPoints.push(null);
		...
		else
		{
			...
			if (this.isBezier)
			{
				for (var i=0; i<points.length-1; ++i)
				{
					if (this.segmentControlPoints[i] == null)
					{
						camera.drawLine(this.points[i], this.points[i+1], this.color, width, dash, fillColorRGBA, this);
					}
					else
					{
						camera.drawBezierCurve(this.points[i], this.segmentControlPoints[i].c0, this.segmentControlPoints[i].c1, this.points[i+1], this.color, width, dash, fillColorRGBA, this);
					}
				}
			}
			else
			{
				camera.drawLineStrip(points, this.color, width, dash, fillColorRGBA, this);
			}
		}
	}
	...
	this.getDragPoints = function(localSpace)
	{
		...
		if (!localSpace)
		{
			for (var i = 0; i != this.pointTypes.length-1; ++i)
			{
				if (this.segmentControlPoints[i] != null)
				{
					if (this.pointTypes[i]>0)
						points.push( this.segmentControlPoints[i].c0 );

					if (this.pointTypes[i+1]>0)
						points.push(this.segmentControlPoints[i].c1);
				}
			}
		}
	...
	this.drawDragPoint = function(camera, index, localSpace)
	{
		...
		else
		{
			...
			else
			{
				...
				if (index%2)
				camera.drawRectangle(this.points[Math.floor(index/2) + index%2], camera.invScale(10), "#00ff00", 2);
			}
		}
	}
	...
	this.toggleDragPoint = function(index)
	{
		if (index < this.points.length)
		{
			this.pointTypes[index] = (this.pointTypes[index] + 1) % 3;

			if (this.pointTypes[index]>0 && this.segmentControlPoints[index] == null)
			{
				this.segmentControlPoints[index] = new Object();

				if (index != this.points.length - 1)
					this.segmentControlPoints[index].c0 = this.points[index + 1].copy();
				else
					this.segmentControlPoints[index].c0 = this.points[index].copy();


				if (index != 0)
					this.segmentControlPoints[index - 1].c1 = this.points[index - 1].copy();
				else
					this.segmentControlPoints[index - 1].c1 = this.points[index].copy();
			}
		}
...
function Rectangle(center)
{
	...
	this.draw = function(camera)
	{
		...
		camera.drawLineStrip([this.points[0], this.points[1], this.points[2], this.points[3], this.points[0]], this.color, width, [], fillColorRGBA, this);
	...
	this.getProperties = function()
	{
		return	[
					...
					{name: "Fill Alpha", control: new Slider(0, 1, this.fillAlpha, 0.1, function (value) { this.fillAlpha = value; lastRectAlpha = this.fillAlpha; this.onChange(); }.bind(this)) },
					{name: "Width", control: new Slider(0, 5, this.width, 1, function (value) { this.width = value; this.onChange(); }.bind(this)) },
					{name: "Rows", control: new Slider(1, 10, this.rows, 1, function (value) { this.rows = value; this.onChange(); }.bind(this)) },
					{name: "Columns", control: new Slider(1, 10, this.columns, 1, function (value) { this.columns = value; this.onChange(); }.bind(this)) },
...
function Dimension(A, B, center)
{
	...
	this.draw = function(camera)
	{
		...
		if (this.center !== undefined)
		{
			...
			camera.drawLineStrip(points, "#000000", width);
		}
		else
		{
			var tan = transpose(sub(this.B, this.A).unit());
...
function BarChart(A, B)
{
	...
	this.draw = function(camera)
	{
		...
		//for (var mip=0; mip<=this.showMip; ++mip)
		{
			...
			for (var i=0; i!=count; ++i)
			{
				...
				camera.drawLineStrip([min1, max1, max2, min2, min1], this.color, width, [], "rgba(0,0,0,0.2)", this);
			}
		}
		...
		// Mip 0
		{
			...
			camera.drawLineStrip(points, this.color, width, [], fillColorRGBA, this);
		}
	}
	...
	this.getProperties = function()
	{
		return	[
					...
					{name: "Values Function", control: new TextBox(this.valuesFunctionStr, function (value) { this.valuesFunctionStr = value; this.onChange(); }.bind(this)) }
				];
	}
...
function ScreenshotArea(p, okCallback, cancelCallback)
{
	...
	this.draw = function(camera)
	{
		...
		camera.drawLineStrip([this.points[0], this.points[1], this.points[2], this.points[3], this.points[0]], "#FF0000", width, []);
	}
