...
function PointLight(O, radius)
{
	...
	this.draw = function(camera)
	{
		...
		camera.drawStar(this.O, 7, this.bulbRadius, 0.5, "#FFC000", width, "rgba(255,255,0,0.8)");
		camera.drawArc(this.O, this.bulbRadius*0.5, 0, Math.PI*2, "#FFC000", width, "rgba(255,255,0,1)");
		...
		if (this.collisionOutline)
		{
			...
			for (var a=0; a<=Math.PI*2; a+=angleStep)
			{
				...
				if (points.length>1)
				{
					camera.drawLine(points[points.length-2], points[points.length-1], "#FFC000", width);
				}
			}

			if (points.length>1)
			{
				camera.drawLine(points[points.length-1], points[0], "#FFC000", width);
			}
		}
		else
		{
			camera.drawArc(this.O, this.radius, 0, Math.PI * 2, "#FFC000", 1, "rgba(0,0,0,0)");
		}
	}
...
function SpotLight(O, target, fovDegrees)
{
	...
	this.draw = function(camera)
	{
		...
		camera.drawLine(add(this.O, mul(tan, this.nearWidth/2)), add(this.O, mul(tan, -this.nearWidth/2)), "#FFC000", width);
		camera.drawLine(add(this.target, mul(tan, this.farWidth/2)), add(this.target, mul(tan, -this.farWidth/2)), "#FFC000", width);

		camera.drawLine(add(this.O, mul(tan, this.nearWidth/2)), add(this.target, mul(tan, this.farWidth/2)), "#FFC000", width);
		camera.drawLine(add(this.O, mul(tan, -this.nearWidth/2)), add(this.target, mul(tan, -this.farWidth/2)), "#FFC000", width);

		camera.drawStar(this.O, 7, 0.5, 0.5, "#FFC000", width, "rgba(255,255,0,0.8)");
	}
