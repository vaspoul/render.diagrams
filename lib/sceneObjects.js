...
function BRDFRay(O, dir)
{
	...
	this.draw = function(camera)
	{
		...
		for (var r=0; r<rays.length; ++r)
		{
			...
			if (bestHit.tRay<1000)
			{
				var metalness = (bestHit.metalness !== undefined) ? bestHit.metalness : 0;
				var roughness = (bestHit.roughness !== undefined) ? bestHit.roughness : 0;
				var BRDF = (bestHit.BRDF !== undefined) ? bestHit.BRDF : Phong;
				var intensity = (bestHit.intensity !== undefined) ? bestHit.intensity : 1;
				...
				if (this.showBRDF)
				{
					var F0 = 0.04 + metalness * (1 - 0.04);
					camera.drawBRDFGraph(BRDF, ray.dir.unit().neg(), bestHit.N, F0, roughness, bestHit.P, intensity);
				}
...
function CameraObject(O, target, fovDegrees)
{
	...
	this.onChange = function()
	{
		...
		this.apexPoint = mad(dir, -apexDistance, this.O);
...
function BRDFHemisphere(centerPos, radius, normal)
{
	...
	this.draw = function(camera)
	{
		...
		if (this.showBRDF)
		{
			...
			camera.drawBRDFGraph(this.BRDF, this.normal, this.normal, F0, this.roughness, this.center, this.intensity);
		}
