...
function SpotLight(O, target, fovDegrees)
{
	...
	this.draw = function(camera)
	{
		...
		if (this.collisionOutline)
		{
			camera.drawLine(add(this.O, mul(tan, this.nearWidth/2)), add(this.O, mul(tan, -this.nearWidth/2)), this.color, width, [5,5]);
			camera.drawLine(add(this.target, mul(tan, this.farWidth/2)), add(this.target, mul(tan, -this.farWidth/2)), this.color, width, [5,5]);

			camera.drawLine(add(this.O, mul(tan, this.nearWidth/2)), add(this.target, mul(tan, this.farWidth/2)), this.color, width, [5,5]);
			camera.drawLine(add(this.O, mul(tan, -this.nearWidth/2)), add(this.target, mul(tan, -this.farWidth/2)), this.color, width, [5,5]);
		...
		if (this.pixelCount > 0)
		{
			...
			for (var fc=1, fx=-this.farWidth/2+farStep, nx = -this.nearWidth/2+nearStep; fc<this.pixelCount*2; ++fc, fx+=farStep, nx+=nearStep)
			{
				...
				if (fc % 2)
				{
					...
					{
						camera.drawLine(pointNear, pointFar, "rgba(0,0,0,0.2)", width, [10,5,2,5]);
					}
				}
				else
				{
					camera.drawLine(pointNear, pointFar, this.color, width);
				}
			}
		}
