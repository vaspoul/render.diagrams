...
function TransformRect(objects)
{
	...
	this.UpdateExtents = function()
	{
		...
		for (var i=0; i<this.objects.length; ++i)
		{
			if (this.objects[i].getBoundsMin !== undefined)
			{
				var objOBBMin = new Vector( dot(this.objects[i].getBoundsMin(), rotMtxCol0abs), dot(this.objects[i].getBoundsMin(), rotMtxCol1abs) );
				var objOBBMax = new Vector( dot(this.objects[i].getBoundsMax(), rotMtxCol0abs), dot(this.objects[i].getBoundsMax(), rotMtxCol1abs) );

				this.boundsMin = min(this.boundsMin, objOBBMin);
				this.boundsMax = max(this.boundsMax, objOBBMax);
			}
		}
	...
	this.draw = function(camera)
	{
		...
		camera.drawArrow(avg(this.points[0], this.points[2]), add(avg(this.points[0], this.points[2]), rotate(new Vector(camera.invScale(70), 0), this.angle * Math.PI / 180)), "#FF0000", 2);
		camera.drawArrow(avg(this.points[0], this.points[2]), add(avg(this.points[0], this.points[2]), rotate(new Vector(0, camera.invScale(70)), this.angle * Math.PI / 180)), "#00FF00", 2);
	}
	...
	this.getOrigin = function()
	{
		return avg(this.boundsMin, this.boundsMax);
	}
