...
function Wall(_points)
{
	...
	this.draw = function(camera)
	{
		...
		for (var i = 0; i != this.points.length - 1; ++i)
		{
			...
			if (this.roughness == 0)
			{
				camera.drawLine(this.points[i], this.points[i + 1], "#000000", width, [], true);
			}
			else
			{
				var l0 = div(L, 60);
...
function BRDFRay(O, dir)
{
	...
	this.draw = function(camera)
	{
		...
		for (var r=0; r<rays.length; ++r)
		{
			...
			if (bestHit.tRay<1000)
			{
				...
				camera.drawLine(ray.start, bestHit.P, "#000000", 2, [5,5], true);
	...
	this.getSnapPoints = function()
	{
		var points = this.intersectionPoints;
		points.push(this.O);
...
function SpotLight(O, target, fovDegrees)
{
	...
	this.draw = function(camera)
	{
		...
		if (this.collisionOutline)
		{
			camera.drawLine(add(this.O, mul(tan, this.nearWidth/2)), add(this.O, mul(tan, -this.nearWidth/2)), this.color, width, [5,5], true);
			camera.drawLine(add(this.target, mul(tan, this.farWidth/2)), add(this.target, mul(tan, -this.farWidth/2)), this.color, width, [5,5], true);

			camera.drawLine(add(this.O, mul(tan, this.nearWidth/2)), add(this.target, mul(tan, this.farWidth/2)), this.color, width, [5,5], true);
			camera.drawLine(add(this.O, mul(tan, -this.nearWidth/2)), add(this.target, mul(tan, -this.farWidth/2)), this.color, width, [5,5], true);
			...
			for (var fc=0, fx=-this.farWidth/2, nx = -this.nearWidth/2; fc<=farStepCount; ++fc, fx+=farStep, nx+=nearStep)
			{
				...
				camera.drawLine(points[points.length-2], points[points.length-1], this.color, width, [], points.length==2);
			}

			points.push(add(this.O, mul(tan, this.nearWidth / 2)));
			camera.drawLine(points[points.length-2], points[points.length-1], this.color, width, [], true);


			if (points.length>1)
			{
				camera.drawLine(points[points.length-1], points[0], this.color, width, [], true);
			}
		}
		else
		{
			camera.drawLine(add(this.O, mul(tan, this.nearWidth/2)), add(this.O, mul(tan, -this.nearWidth/2)), this.color, width, [], true);
			camera.drawLine(add(this.target, mul(tan, this.farWidth/2)), add(this.target, mul(tan, -this.farWidth/2)), this.color, width, [], true);

			camera.drawLine(add(this.O, mul(tan, this.nearWidth/2)), add(this.target, mul(tan, this.farWidth/2)), this.color, width, [], true);
			camera.drawLine(add(this.O, mul(tan, -this.nearWidth/2)), add(this.target, mul(tan, -this.farWidth/2)), this.color, width, [], true);
		}
		...
		if (this.pixelCount > 0)
		{
			...
			for (var fc=1, fx=-this.farWidth/2+farStep, nx = -this.nearWidth/2+nearStep; fc<this.pixelCount*2; ++fc, fx+=farStep, nx+=nearStep)
			{
				...
				if (fc % 2)
				{
					...
					{
						camera.drawLine(pointNear, pointFar, "rgba(0,0,0,0.2)", width, [10,5,2,5], true);
					}
				}
				else
				{
					camera.drawLine(pointNear, pointFar, this.color, width, [], true);
				}
			}
		}
...
function Line(A, B)
{
	...
	this.draw = function(camera)
	{
		...
		camera.drawLine(this.A, this.B, this.color, width, [], true);
	}
