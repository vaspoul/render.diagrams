...
function Wall(_points)
{
	...
	this.setDragPointPos = function(index, p, localSpace, dragStartPoint, camera)
	{
		...
		if (camera != undefined)
		{
			...
			if (index == 0)
			{
				p0 = this.points[0].copy();
				p1 = this.points[1].copy();
			}
			else
			{
				p0 = this.points[index-1].copy();
				p1 = this.points[index].copy();
			}
			...
			var p = mad(lx, ((p1.x < p0.x) ? +1 : -1) * camera.invScale(10), p);
			p = mad(ly, ((p1.x < p0.x) ? -1 : +1) * camera.invScale(10), p);

			var angle = 0;//toAngle(delta.unit());
