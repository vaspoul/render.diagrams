...
function Wall(_points)
{
	...
	this.draw = function(camera)
	{
		...
		for (var i = 0; i != count; ++i)
		{
			...
			if (this.roughness == 0)
			{
				camera.drawLine(this.points[i], this.points[(i+1)%this.points.length], this.color, width, [], this);
			}
			else
			{
				var l0 = div(L, 60);
				...
				for (var j=0; j<60; ++j)
				{
					...
					camera.drawLine(add(add(this.points[i], mul(l0, j)), mul(t0,lastRand)), add(add(this.points[i], mul(l0, j + 1)), mul(t0,newRand)), this.color, width);
					lastRand = newRand;
				}
			}

			{
				...
				{
					hatchPoints.push(add(add(this.points[i], mul(l, j * stepUnits)), mul(t, 0)));
					hatchPoints.push(add(add(this.points[i], mul(l, (j - 1) * stepUnits)), mul(t, stepUnits)));
				}
...
function CameraObject(O, target, fovDegrees)
{
	...
	this.draw = function(camera)
	{
		...
		var apexDistance = this.nearWidth / tanFOV;
							...
							{ name: "FOV", control: new Slider(0, 180, this.fovDegrees, 5,	function (value) 
																							{
																								...
																								var tanFOV = Math.tan(this.fovDegrees * Math.PI / 180);
...
function Line(_points)
{
	...
	this.draw = function (camera)
	{
		...
		{
			// Trully lazy!
			camera.drawArrow(avg(this.points[0], this.points[1]), this.points[1], this.color, width, [], this);
			camera.drawArrow(avg(this.points[0], this.points[1]), this.points[0], this.color, width, [], this);
		}
		else
		{
			var count = this.closed ? this.points.length : this.points.length - 1;
			...
			if (this.arrowEnd)
			{
				camera.drawArrow(points[points.length-2], points[points.length-1], this.color, width, [], this);
			}

			if (this.arrowStart)
			{
				camera.drawArrow(points[1], points[0], this.color, width, [], this);
			}
		}
	}
	...
	[
		...
		{name: "Width", control: new Slider(1, 5, this.width, 1, function (value) { this.width = value; this.onChange(); }.bind(this)) },
...
function Rectangle(center)
{
	...
	this.saveAsJavascript = function()
	{
		...
		str += "rectObj.subdivisions = " + this.subdivisions + ";\n";
	...
	this.draw = function(camera)
	{
		...
		{
			var deltaX0 = div(sub(this.points[1], this.points[0]), this.subdivisions);
			var deltaX1 = div(sub(this.points[2], this.points[3]), this.subdivisions);
			var deltaY0 = div(sub(this.points[3], this.points[0]), this.subdivisions);
			var deltaY1 = div(sub(this.points[2], this.points[1]), this.subdivisions);

			var points = [];

			for (var i = 1; i <= this.subdivisions - 1; ++i)
			{
	...
	this.getSnapPoints = function()
	{
		...
		if (this.subdivisions > 1)
		{
			...
			for (var i = 1; i <= this.subdivisions - 1; ++i)
			{
				...
				var deltaY = div(sub(pX1, pX0), this.subdivisions);

				for (var j = 1; j <= this.subdivisions - 1; ++j)
				{
					var p = mad(deltaY, j, pX0);

					points.push({type: "intersection", p: p });
				}
			}
		}
	...
	[
		...
		{name: "Fill Color", control:new ColorPicker(this.fillColor, function(value){ this.fillColor = value; lastRectFillColor = this.fillColor; this.onChange(); }.bind(this) )},
		{name: "Fill Alpha", control: new Slider(0, 1, this.fillAlpha, 0.1, function (value) { this.fillAlpha = value; lastRectAlpha = this.fillAlpha; this.onChange(); }.bind(this)) },
		{name: "Subdivisions", control: new Slider(0, 10, this.subdivisions, 1, function (value) { this.subdivisions = value; this.onChange(); }.bind(this)) },
  	];
...
function Group(objects)
{
	...
	this.deleteObjects = function(objectList)
	{
		...
		{
			if (objectList.indexOf(this.objects[this.objects.length-1-i])>=0)
			{
				this.scene.addObject(this.objects[this.objects.length-1-i], index);
