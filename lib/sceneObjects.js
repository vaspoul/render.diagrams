...
function TransformRect(objects)
{
	...
	this.updateExtents = function(updateCenter)
	{
		...
		if (updateCenter)
		{
			this.center = avg(this.points[0], this.points[2]);
		}
	}
	...
	this.draw = function(camera)
	{
		...
		camera.drawArrow(this.center, add(this.center, rotate(new Vector(camera.invScale(70), 0), this.angle * Math.PI / 180)), "#FF0000", 2);
		camera.drawArrow(this.center, add(this.center, rotate(new Vector(0, camera.invScale(70)), this.angle * Math.PI / 180)), "#00FF00", 2);
	}
	...
	this.getSnapPoints = function()
	{
		...
		var midpoint = avg( avg(this.points[0], this.points[1]), avg(this.points[2], this.points[3]) );

		snaps.push({ type: "node", p: midpoint });
	...
	this.getDragPoints = function(localSpace, camera)
	{
		...
		for (var i = 0; i < 8; ++i)
		{
			points.push(add(this.center, rotate(new Vector(camera.invScale(70), 0), (this.angle + i*360/8) * Math.PI / 180)));
		}
	...
	this.drawDragPoint = function(camera, index, localSpace)
	{
		...
		if (index>=9)
		{
			camera.drawArc(this.center, camera.invScale(70), 0, Math.PI*2, "#ff0000", 2, undefined, [5,5]);
		}
	}
	...
	this.setDragPointPos = function(index, p, localSpace)
	{
		...
		{
			this.angle = toAngle(sub(p, this.center)) * 180 / Math.PI;
