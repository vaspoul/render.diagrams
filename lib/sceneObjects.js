...
function FunctionGraph(origin)
{
	...
	this.evaluateOnMouseCursor	= false;
	...
	this.draw = function(camera, mousePos)
	{
		...
		if (this.evaluateOnMouseCursor)
		{
			if (this.coordinateType == 0)
			{
				...
				var mouseY = this.yFunction(mouseX);
				...
				if (mouseX >= this.xMin && mouseX <= this.xMax)
				{
					...
					pY = add(pY, mul(this.yAxis, mouseY));

					var p = this.graphOrigin.copy();
					p = add(p, mul(this.xAxis, mouseX));
					p = add(p, mul(this.yAxis, mouseY));
			...
			else
			{
				...
				if (mouseY>0)
				{
					var radial = mul(fromAngle(mouseX), mouseY);
					...
					camera.drawArc(this.origin, camera.invScale(50), 0, mouseX, "#000000", 1, undefined, [5,5]);
					camera.drawArc(p, camera.invScale(5), 0, Math.PI*2, "#008000", 4);
					camera.drawArrow(this.origin, p, 8, "#000000", 1);

					var right = (mouseX <= Math.PI/2) && (mouseX >= Math.PI*3/2);

					var p = p.copy();
					p.x += (right ? +1 : -1) * camera.invScale(10);
					p.y += camera.invScale(10);
