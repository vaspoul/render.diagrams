...
function Wall(_points)
{
	...
	this.draw = function(camera)
	{
		...
		camera.drawLines(linePoints, this.appearance.GetLineColor(), this.appearance.GetLineWidth(this.selected), this.appearance.GetLineDash(), undefined, this);
	...
	this.rayHitTest = function(rayPos, rayDir)
	{
		...
		for (var i = 0; i != count; ++i)
		{
			...
			if (hitResult.hit)
			{
				...
				hitResult.color = this.appearance.GetLineColor();
...
function ArcWall(centerPos, radius, startAngle, endAngle)
{
	...
	this.draw = function(camera)
	{
		...
		camera.drawArc(this.center, this.radius, this.startAngle, this.endAngle, this.appearance.GetLineColor(), this.appearance.GetLineWidth(this.selected));
	...
	this.rayHitTest = function(rayPos, rayDir)
	{
		...
		if (perpDist < this.radius)
		{
			...
			results[0].P = P;
			results[0].color = this.appearance.GetLineColor();
...
function PointLight(O, radius)
{
	...
	this.draw = function(camera)
	{
		...
		{
			camera.drawArc(this.center, this.radius, 0, Math.PI * 2, this.appearance.GetLineColor(), 1, "rgba(0,0,0,0)");
		}
	}
...
function Rectangle(center)
{
	...
	this.draw = function(camera)
	{
		camera.drawLineStrip(this.points, true, this.appearance.GetLineColor(), this.appearance.GetLineWidth(this.selected), [], this.appearance.GetFillColor(), this);
...
function NGon(centerPos, radius)
{
	...
	this.draw = function(camera)
	{
		...
		{
			camera.drawArc(this.center, this.radius, 0, Math.PI*2, this.appearance.GetLineColor(), this.appearance.GetLineWidth(this.selected), this.appearance.GetFillColor(), this.appearance.GetLineDash());
		}
		else
		{
...
// Function Graph
// ----------------------------------------------------------------------------------------------------------------
var lastFunctionGraphAppearance = new Appearance("#4286f4", 1, 2, "#00FF00", 0,2);
...
function FunctionGraph(origin)
{
	...
	this.functionStr		= "return Math.sin(x/3.14159*2)*5;";
	this.onChangeListeners	= [];
	this.boundsMin			= new Vector(Number.MAX_VALUE,Number.MAX_VALUE);
	this.boundsMax			= new Vector(-Number.MAX_VALUE,-Number.MAX_VALUE);
	...
	this.draw = function(camera)
	{
		...
		// Axes
		{
			camera.drawArrow(	add(this.origin, mul(this.xAxis, this.graphOriginLocal.x + this.xMin)), 
								...
								15, "#000000", this.selected ? 4 : 1, [], this);
		...
		// Graph
		{
			...
			// Create line list (line *list* rather than strip so that we can easily clip against the y limits) 
			// (TODO: can improve performance here, we're evaluating the function 2x as necessary)
			for (var x = this.xMin; x < this.xMax; x += xStep)
			{
				var x0 = x;
				...
				lastX = x1;
				//camera.drawRectangle(p0, camera.invScale(10), this.appearance.GetLineColor(), 2);
			}

			// Vertical line to close the shape
			if (points.length > 0)
			{
				var last = points[points.length-1].copy();

				var o = this.graphOrigin.copy();
				o = add(o, mul(this.xAxis, lastX));

				points.push(last);
				points.push(o);
			}

			camera.drawLines(points, this.appearance.GetLineColor(), this.appearance.GetLineWidth(this.selected), this.appearance.GetLineDash(), this.appearance.GetFillColor(), this);
		}
	}
...
function Text(anchorPoint, initialText)
{
	...
	this.draw = function(camera)
	{
		...
		for (var i=0; i!=this.lines.length; ++i)
		{
			...
			camera.drawText(p, this.lines[i], this.appearance.GetLineColor(), "left", this.angle, font);
		}
	}
