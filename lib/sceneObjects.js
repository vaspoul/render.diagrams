...
function FunctionGraph(origin)
{
	...
	this.onChange = function()
	{
		...
		try
		{
			this.yFunction = Function("x", this.functionStr);
		}
		catch (e)
		{
	...
	this.draw = function(camera)
	{
		...
		// Axis labels
		{
			...
			// x axis
			if (this.coordinateType == 0 && this.xLabel != 2)
			{
		...
		// Graph
		{
			...
			if (this.appearance.NeedOutline() && valuesClipped)
			{
				...
				for (var x = this.xMin; x < this.xMax; x += evalStep)
				{
					...
					var y0 = this.yFunction(x0);
					var y1 = this.yFunction(x1);
...
function BouncingBall(position)
{
	...
	this.visible				= true;
	this.frozen					= false;
	this.appearance				= new Appearance("#000000", 1, 1, "#FF0000", 0.5);
	this.onChangeListeners		= [];
	this.boundsMin				= new Vector(Number.MAX_VALUE,Number.MAX_VALUE);
	this.boundsMax				= new Vector(-Number.MAX_VALUE,-Number.MAX_VALUE);
	...
	this.saveAsJavascript = function()
	{
		...
		str += "bouncingBall.mass = " + this.mass + ";\n";
	...
	this.onFrameTick = function(deltaTime_ms)
	{
		...
		// Collision check
		var bestHit = this.scene.rayHitTest(this.currentPosition, this.currentVelocity.unit());

		// Are we going to hit something before the movement is done?
		if (bestHit.hit && (bestHit.tRay - this.radius) <= length(positionDelta))
		{
			this.currentPosition = sub(bestHit.P, mul(this.currentVelocity.unit(), this.radius));
			this.currentVelocity = mul(reflect(this.currentVelocity, bestHit.N), this.bounceFactor);
		}
		else
		{
	...
	this.hitTest = function(a, b, camera)
	{
		...
		if (	b.x>=this.currentPosition.x &&
				a.x<=this.currentPosition.x &&
				b.y>=this.currentPosition.y &&
				a.y<=this.currentPosition.y)
		{
			return true;
		}

		var p = add(this.currentPosition, mul(sub(rectCenter, this.currentPosition).unit(), this.radius));
	...
	this.getProperties = function()
	{
		...
		return	[
					...
					{ name: "Wrap Around", control: new TickBox(this.wrapAround, function (value) { this.wrapAround = value; this.onChange(); }.bind(this)) },
				];
	}
	...
	this.updateTimer = setInterval( function() { this.onFrameTick() }.bind(this), 30 );
}
