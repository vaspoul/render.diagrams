		...
		for (var mip=0; mip<=this.pixelMip; ++mip)
		{
			...
			for (var i = 0; i < points.length - 1; ++i)
			{
				...
				while (t < 1)
				{
					...
					camera.drawRectangle(r0, r1, this.color, width, [], fillColorRGBA);
					...
					var stepsToBoundaryXY = new Vector(1000, 1000);

					stepsToBoundaryXY.x = Math.abs(L.x)>0 ? div(sub(boundary, p).x, L.x) : 1000;
					stepsToBoundaryXY.y = Math.abs(L.y)>0 ? div(sub(boundary, p).y, L.y) : 1000;

					var stepsToBoundary = Math.min(stepsToBoundaryXY.x, stepsToBoundaryXY.y);

					stepsToBoundary += 0.01;
...
function BarChart(A, B)
{
	...
	this.draw = function(camera)
	{
		...
		if (this.showMip>0)
		{
			var mip0count = Math.pow(2, this.showMip);
