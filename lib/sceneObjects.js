...
function Wall(pointA, pointB)
{
	...
	this.setDragPointPos = function(index, p, localSpace)
	{
		if (localSpace)
		{
			var L = sub(this.pointB, this.pointA).unit();

			if (index==0)
			{
				this.pointA = add(this.pointB, mul(dot(sub(p, this.pointB), L.neg()), L.neg()));
			}
			else if (index==1)
			{
				this.pointB = add(this.pointA, mul(dot(sub(p, this.pointA), L), L));
			}
		}
		else
		{
