...
function DiffractionGrating(firstPoint)
{
	...
	this.saveAsJavascript = function()
	{
		...
		{
			str += "diffractionGrating.name = \"" + this.name + "\";\n";
		}
			...
			if (lightObject != undefined)
			{
				if (this.showCameraLines && cameraObject != undefined)
				{
					for (var i=0; i <= count; ++i)
					{
						...
						var pCamDir = normalize(sub(cameraObject.O, p));
						...
						for (var band=-2; band<=2; ++band)
						{
							...
							{
								camera.drawLine( p, cameraObject.O, wavelengthToColor(wavelength)[3], 4, abs(band)>1 ? [5,5] : undefined);
				...
				if (this.showDebugPoint)
				{
					...
					for (var band=-1; band<=1; ++band)
					{
						...
						if (this.showCameraLines && cameraObject != undefined)
						{
							var pCamDir = normalize(sub(cameraObject.O, debugPoint));
							...
							if (!this.restrictToCamFOV || dot(pCamDir.neg(), cameraObject.dir) >= cameraFOVcosine)
							{
								...
								if (wavelength >= 380 && wavelength <= 680)
								{
									camera.drawLine( debugPoint, cameraObject.O, wavelengthToColor(wavelength)[3], 4, abs(band)>1 ? [5,5] : undefined);
