...
function Button(imgSrc, sizeOverride, textLabel, callback)
{
	...
	this.addControls = function(parentElement)
	{
		...
		if (textLabel)
		{
			if (textLabel.startsWith("{hint}"))
			{
				this.img.title = textLabel.substring(6);
			}
			else
			{
...
function MultiToolButton(sizeOverride)
{
	...
	this.showPopout = function(show)
	{
		if (show == undefined || show)
		{
			...
			if ( (rect.right + divRect.right - divRect.left) >= window.innerWidth )
			{
				this.popoutDiv.style.left = rect.left - (divRect.right - divRect.left) - 10;
			}
			else
			{
			...
			if ( (rect.top + divRect.bottom - divRect.top) >= window.innerHeight )
			{
				this.popoutDiv.style.top = rect.top - (divRect.bottom - divRect.top);
			}
			else 
			{
				this.popoutDiv.style.top = rect.top;
