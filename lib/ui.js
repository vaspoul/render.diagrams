...
function PopoutSlider2(minVal, maxVal, initialVal, stepVal, callback, popoutDirection)
{
...
function PopoutSlider(minVal, maxVal, initialVal, stepVal, callback, popoutDirection)
{
	...
	this.updateValue = function(newValue)
	{
		...
		this.label.innerHTML = currentValue.toFixed(2);
		this.editBox.value = currentValue.toFixed(2);
		this.slider.value  = currentValue / stepVal;
	...
	this.showPopout = function()
	{
		...
		// Enable it off screen so that we get the correct dimensions below
		this.popoutDiv.style.display = "inline";
		this.popoutDiv.style.left = window.innerWidth + 100;
		...
		{
			this.popoutDiv.style.top = anchorPoint[3] + 7;
			...
			{
				this.popoutDiv.style.left = (anchorPoint[0] + anchorPoint[2]) * 0.5 - divWidth * 0.5;
			}
		}
		else if (popoutDirection == "top") // bottom edge
		{
		...
		if (g_popoutElementVisible != this.popoutDiv)
		{
			if (g_popoutElementVisible != undefined)
			{
				g_popoutElementVisible.style.display = "none";
		...
		g_popoutElementVisible = this.popoutDiv; 
	}

	this.hidePopout = function()
	{
		this.popoutDiv.style.display = "none";
	...
	this.addControls = function (parentElement)
	{
		...
		{
			if (document.activeElement != this.editBox)
			{
				this.timeout = setTimeout( function()
				{
					this.hidePopout();
		...
		this.popoutDiv.style.padding = "4px";
		this.popoutDiv.style.border = "1px solid #DDDDDD";
		this.popoutDiv.style.boxShadow = "3px 3px 5px #999999";
		this.popoutDiv.onmouseenter = function()
		{
			clearTimeout(this.timeout);

		}.bind(this);

		this.popoutDiv.onmouseleave = function()
		{
			if (document.activeElement != this.editBox)
			{
				this.timeout = setTimeout( function()
		...
		this.editBox.onmousedown = function() 
		{
			clearTimeout(this.timeout);
		}.bind(this);

		this.popoutDiv.appendChild(document.createTextNode(minVal + " "));
		this.popoutDiv.appendChild(this.slider);
		this.popoutDiv.appendChild(document.createTextNode(" " + maxVal));
		var spacerElement = document.createElement("span");
		spacerElement.className = "toolbar";
		spacerElement.style.margin = 4;
		this.popoutDiv.appendChild(spacerElement);

		this.popoutDiv.appendChild(this.editBox);
