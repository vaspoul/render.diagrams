function Scene()
{
	...
	this.getSnapPoint = function(mousePos, extraSnapPoints, threshold, ignoreList)
	{
		var bestPoint = null;
		...
		for (var i=0; i<this.objects.length; ++i)
		{
			...
			for (var p=0; p<points.length; ++p)
			{
				...
				{
					bestDistance = d;
					bestPoint = {type: points[p].type, p:points[p].p};
				}
			}
		}
		...
		for (var p=0; p<extraSnapPoints.length; ++p)
		{
			...
			{
				if (ignoreList.indexOf(extraSnapPoints[p].objects[o]) >= 0)
				{
					skip = true;
					break;
				}
			...
			{
				bestDistance = d;
				bestPoint = {type: extraSnapPoints[p].type, p:extraSnapPoints[p].p};
			}
		}
