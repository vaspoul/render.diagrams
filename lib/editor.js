function GeneralDrawing(docTag)
{
	...
	function onMouseDown(evt)
	{
		...
		if (evt.buttons & 1)
		{
			...
			if (tool == "select")
			{
				...
				if (evt.altKey == 0)
				{
					snapPoint = scene.getSnapPoint(lastMousePos, camera.getSnapPoints(lastMousePos), camera.invScale(30), enableSnap, [objectBeingMade]);
		...
		if (evt.button == 0) // object move or marquee
		{
			...
			else if (tool == "addWall" || tool == "addArcWall" || tool == "addRay" || tool == "addPointLight" || tool == "addSpotLight" || tool == "addParallelLight" || tool == "addCamera" || tool == "addLine" || tool == "addRect" || tool == "addHemisphere" || tool == "addNGon" || tool == "addLens" || tool == "addCapsule" || tool == "addPoint" || tool == "addDimension" || tool == "addBarChart" || tool == "addFunctionGraph" || tool == "takeScreenshot" || tool == "addText" || tool == "addAxis" || tool == "addPageOutline" || tool == "addImage" || tool == "addBouncingBall" || tool == "addDiffractionGrating")
			{
				...
				if (evt.altKey == 0)
				{
					...
					snapPoint = scene.getSnapPoint(lastMousePos, camera.getSnapPoints(lastMousePos, previousMousePos), camera.invScale(30), enableSnap, [objectBeingMade]);
	...
	function onMouseMove(evt)
	{
			...
			if (mode == "move")
			{
				...
				if (evt.altKey == 0 || tool == "transform")
				{
					...
					if (dragPoint != undefined && dragPoint.object != undefined && dragPoint.object.allowSelfSnap)
						snapPoint = scene.getSnapPoint(lastMousePos, camera.getSnapPoints(posForSnapSearch, previousMousePos, mouseDir), camera.invScale(30), enableSnap, ignoreList, undefined, undefined, { object: dragPoint.object, index: dragPoint.index, localSpace: dragPoint.local });
					else
						snapPoint = scene.getSnapPoint(lastMousePos, camera.getSnapPoints(posForSnapSearch, previousMousePos, mouseDir), camera.invScale(30), enableSnap, ignoreList);
		...
		//		snapPoint = scene.getSnapPoint(lastMousePos, camera.getSnapPoints(lastMousePos, previousMousePos, mouseDir), camera.invScale(30), enableSnap, ignoreList);
