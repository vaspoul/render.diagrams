function GeneralDrawing(docTag)
{
	...
	function setup()
	{
		...
		// Code box
		{
			...
			codeBoxDock.addEventListener('keyup', function(evt)
														{
															if (evt.keyCode==27) // ESC
															{
	...
	function saveAsImage(formatIndex, popout)
	{
	...
	function saveAsSVGxxx()
	{
		var center = avg(scene.getBoundsMin(), scene.getBoundsMax());
		var extents = sub(scene.getBoundsMax(), scene.getBoundsMin());

		var popoutCanvas = document.createElement('canvas');
		popoutCanvas.height = 800;
		popoutCanvas.width = popoutCanvas.height * extents.x / extents.y;
		
		var popoutGrid = new Grid()
		popoutGrid.spacing = grid.spacing;

		var popoutCamera = new Camera(popoutCanvas, true);
		...
		popup.document.write("<div style=\"width: " + popoutCanvas.width + "px; display: block; margin:auto auto;\">\n");

		popup.document.write("<h2>Embedded SVG</h2>\n");
		popup.document.write("<p>Click to download</p>\n");
		popup.document.write("<div id=\"container\" style=\"width: " + popoutCanvas.width + "px; height: " + popoutCanvas.height + "px; margin:0 auto; border:1px solid black;\">");

		popup.document.write("<a href=\"data:image/svg+xml," + encodedSVG + "\" download=\"" + scene.name + ".svg\">\n");

		popup.document.write("<img src=\"data:image/svg+xml," + encodedSVG + "\">\n");
		popup.document.write("</a>\n");

		popup.document.write("</div>");


		popup.document.write("<h2>Download Link</h2>\n");
		popup.document.write("<a href=\"data:image/svg+xml," + encodedSVG + "\" download=\"" + scene.name + ".svg\">Download SVG</a>\n");

		popup.document.write("<h2>SVG Code</h2>\n");
		popup.document.write("<textarea rows=\"" + Math.max(20, svg.split("\n").length) + "\" style=\"width: 100%; display: block; margin:auto auto; border:1px solid black;\">\n");
