function GeneralDrawing(docTag)
{
	...
	function setup()
	{
		...
		if (scene.objects.length == 0)
		{
			...
			var lineObject = new Line([new Vector(-14, 5), new Vector(-1, 22), new Vector(2, 6), new Vector(-13, 17), new Vector(12, 17)]);
			lineObject.closed = true;
			lineObject.appearance.fillAlpha = 0.2;
			scene.addObject(lineObject);
			
			var grating = new DiffractionGrating(new Vector(-5,-10));
			grating.setDragPointPos(1, new Vector(+5,-10));
			scene.addObject(grating);
			
			var light = new PointLight(new Vector(16, -18), 2);
			scene.addObject(light);

			var cam = new CameraObject(new Vector(0, 0), new Vector(0, -9), 10);
			cam.nearWidth = 3;
			cam.farWidth = 9.551464216791642;
			cam.fovDegrees = 40;
			cam.collisionOutline = false;
			scene.addObject(cam);

			camera.setViewPosition(-2, 10);
			camera.setUnitScale(24);
