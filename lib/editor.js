function GeneralDrawing(docTag)
{
	...
	function setup()
	{
		...
		// File controls
		{
			...
			popoutMenuButton.addControl(new Button("images/open_example.svg", [24,24],				"Open Tutorial",					undefined, function () { loadTutorialScene(scene, camera);	}));
	...
	function onKeyDown(evt)
	{
		...
		else if (evt.keyCode == 70 || evt.keyCode == 113) // F or F2
		{
			zoomSelection();
		}
		...
		for (var i=0; i!=scene.objects.length; ++i)
		{
			...
			function onClickHandler(evt, obj)
			{
				if (evt.shiftKey)
				{
					...
					var index1 = obj.scene.getObjectIndex(this);
