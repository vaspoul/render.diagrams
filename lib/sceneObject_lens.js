...
function Lens(p0, p1)
{
	...
	this.onChange = function(changeDetails)
	{
		...
		this.radiusL = Math.max(this.radiusL, halfHeight);
		this.radiusR = Math.max(this.radiusR, halfHeight);
	...
	this.getProperties = function()
	{
		...
		if (this.symmetrical)
		{
			properties.push( {name: "Radius", control: new Slider(Math.ceil(distance(this.p0, this.p1) * 0.5), 100, this.radiusL, 0.1, function (value) { this.radiusL = value; this.onChange(); }.bind(this)) } );
