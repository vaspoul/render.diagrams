...
function PointLight(O, radius)
{
	...
	this.draw = function(camera)
	{
		...
		if (this.collisionOutline || this.showMinZBuffer || this.showMaxZBuffer)
		{
			this.boundsMin = new Vector(Number.MAX_VALUE,Number.MAX_VALUE);
			this.boundsMax = new Vector(-Number.MAX_VALUE,-Number.MAX_VALUE);
			...
			var angleStep = camera.invScale(10) / (2 * Math.PI * this.radius) * 360 * Math.PI / 180;
			...
			if (this.collisionOutline)
			{
				...
				for (var p=0; p<snapPoints.length; ++p)
				{
					...
					testAngles.push(a - Math.PI * 0.00001);
					testAngles.push(a);
					testAngles.push(a + Math.PI * 0.00001);
				}

				if (this.showMinZBuffer || this.showMaxZBuffer)
				{
			...
			for (var i=0; i!=testAngles.length; ++i)
			{
				...
				if (bestHit.tRay<this.radius)
				{
					...
					var cosDeltaAngle = dot(pixelDir, dir);
					var z = bestHit.tRay * cosDeltaAngle;
		...
		if (this.pixelCount > 0)
		{
			...
			for (var i=0; i<this.pixelCount*2; ++i)
			{
				...
				if (i % 2)
				{
					...
					{
						var minZ = minZValues[Math.floor(i/2)];
					...
					if (this.showMaxZBuffer)
					{
						...
						var maxZ = maxZValues[Math.floor(i/2)];

						if (maxZ < Number.MAX_VALUE)
						{
			...
			if (this.showMinZBuffer)
			{
				camera.drawLineStrip(minPoints, true, "#00FF00", this.appearance.GetLineWidth(this.selected), undefined, undefined, this);
			}

			if (this.showMaxZBuffer)
			{
				camera.drawLineStrip(maxPoints, true, "#0000FF", this.appearance.GetLineWidth(this.selected), undefined, undefined, this);
			}
		}
	}
