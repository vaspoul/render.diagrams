function Camera(mainCanvas)
{
	...
	function init()
	{
		...
		canvas[2].width = mainCanvas.width;
		canvas[2].height = mainCanvas.height;
		canvas[2].style.position = "fixed";
		canvas[2].style.left = mainCanvas.style.left;
		canvas[2].style.top = mainCanvas.style.top;
		canvas[2].style.zIndex = mainCanvas.style.zIndex-1;
		canvas[2].style.cursor = "none";
		canvas[2].tabIndex = -1;
		
		if (mainCanvas.parentNode != undefined)
		{
			mainCanvas.parentNode.insertBefore(canvas[2], mainCanvas);
		}

		graphics = [new Graphics(canvas[0]), new Graphics(canvas[1]), new Graphics(canvas[2])];
	...
	function setLayer(index)
	{
		if (activeLayer != index)
		{
			...
			clientRect = activeCanvas.getBoundingClientRect();
		}
	}
	...
	function drawLine(a,b,color,width,dash,ownerObject)
	{
		...
		return activeGraphics.drawLine(transformP(a), transformP(b), color, width, dash);
	}

	function drawBezierCurve(a,ca,cb,b,color,width,dash,ownerObject)
	{
		//if (ownerObject !== undefined)
		//{
		//	lines.push({a:a, b:b, object:ownerObject});
		...
		return activeGraphics.drawBezierCurve(transformP(a), transformP(ca), transformP(cb), transformP(b), color, width, dash);
	}
	...
	{
		if ( (typeof(b) !== "object") )
			return activeGraphics.drawRectangle(transformP(a), scale(b), color, width, dash, fillColor);
		else
			return activeGraphics.drawRectangle(transformP(a), transformP(b), color, width, dash, fillColor);
	}

	function drawArrow(pStart,pEnd,color,width,dash,ownerObject)
	{
		if (ownerObject !== undefined)
		{
		...
		return activeGraphics.drawArrow(transformP(pStart), transformP(pEnd), color, width, dash);
	}

	function drawText(O,text,color,align, angle, font)
	{
		return activeGraphics.drawText(transformP(O),text,color,align,angle,font);
	}
	...
	{
		return activeGraphics.drawArc(transformP(O), scale(radius), -startAngle, -endAngle, lineColor, lineWidth, fillColor, lineDash);
	}
	...
	function drawBRDFGraph(BRDF, V, N, F0, roughness, O, radius, lineColor, lineWidth, fillColor, lineDash)
	{
		...
		return activeGraphics.drawBRDFGraph(BRDF, transformN(V), transformN(N), F0, roughness, transformP(O), scale(radius), lineColor, lineWidth, fillColor, lineDash);
	}
