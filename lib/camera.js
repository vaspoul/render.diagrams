function Camera(canvas)
{
	...
	function drawText(O,text,color,align, angle)
	{
		return graphics.drawText(transformP(O),text,color,align,angle);
	}
	...
	function getSnapPoints(mousePoint, previousMousePos, mouseDir)
	{
		...
		for (var i=0; i!=lines.length; ++i)
		{
			...
			if (	mousePoint.x > Math.max(A.x, B.x) ||
					mousePoint.x < Math.min(A.x, B.x) ||
					mousePoint.y > Math.max(A.y, B.y) ||
					mousePoint.y < Math.min(A.y, B.y))
			{
				continue;
			}
			...
			if (mousePoint !== undefined)
			{
				...
				//if (previousMousePos !== undefined && mouseDir !== undefined && Math.abs(t)<=this.invScale(30))
				//{
				//	if (dot(mouseDir, tan)<0)
				//	{
				...
				//			var extendPoint = { type: "extendTo", p: intersection.P, testP: testI, objects: [lines[i].object] };

				//					if (intersection.tLine <= 0)		perp.p0 = A;
				//			else	if (intersection.tLine >= 1)		perp.p0 = B;
				//			else										perp.p0 = testI;
