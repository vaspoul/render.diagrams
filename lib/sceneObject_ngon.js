...
function NGon(centerPos, radius)
{
	...
	this.hitTest = function(a, b, camera)
	{
		...
		var angleP = toAngle(sub(rectCenter, this.center)) - this.rotationAngle;

		if (angleP < this.startAngle || angleP > this.endAngle)
			return false;

		var angleStep = Math.PI * 2 / this.sideCount;
		var phi = (Math.PI - angleStep) / 2;
		var radiusFactor = Math.sin(phi) / Math.sin(phi + angleP - Math.floor(angleP/angleStep) * angleStep);
		var pointRadius = this.radius * radiusFactor;

		var p = add(this.center, mul(sub(rectCenter, this.center).unit(), pointRadius));
