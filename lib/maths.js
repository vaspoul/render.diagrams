...
function intersectRayArc(rayStart, rayDir, arcCenter, arcRadius, arcStartAngle, arcEndAngle)
{
	...
	// If ray starts outside the arc, move back
	if (distance(rayStart, arcCenter) > arcRadius)
		delta = -delta;
	...
	// impact point
	var P = add(rayStart, mul(rayDir, distanceToImpact));

	// check if impact point is outside the (stand,end) angle range
	var OP = sub(P, arcCenter);
	var OPAngle = toAngle(OP);

	if (OPAngle < 0)
		OPAngle	+= Math.PI*2;

	var midAngle = (arcStartAngle + arcEndAngle) * 0.5;

	if (Math.cos(OPAngle - midAngle) < Math.cos(arcStartAngle - midAngle))
		return result;

	result.hit = true;
	result.tRay = distanceToImpact;
	result.P = P;
