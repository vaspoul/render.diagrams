...
function overlapRayRect(rayStart, rayDir, rectMin, rectMax, rayRadius)
{
	...
	// distance of box center along ray
	var centerDistance = dot(sub(rectCenter, rayStart), rayDir);

	// point along ray closest to box center
	var closestPoint = add(rayStart, mul(rayDir, centerDistance));

	// move towards center by rayRadius
	var vecToCenter = sub(rectCenter, closestPoint).unit();
	closestPoint = add(closestPoint, mul(vecToCenter, rayRadius));

	if (	closestPoint.x < rectMin.x ||
