...
function Frustum()
{
	...
	this.saveAsJavascriptBase = function(objectName, str)
	{
		...
		str += objectName + ".hitBackfaces = " + this.hitBackfaces + ";\n";
	...
	this.draw = function(camera)
	{
		...
		if (this.collisionOutline || this.showMinZBuffer)
		{
			...
			// Add steps at each snap point so that we don't miss any corners
			{
				...
				for (var p=0; p<snapPoints.length; ++p)
				{
					...
					var d0 = dot(sub(snapPoints[p], this.O), tan);
					var d1 = d0 - delta;
					var d2 = d0 + delta;
					
					var nearD0 = this.nearZ / z * d0;
					var nearD1 = this.nearZ / z * d1;
					var nearD2 = this.nearZ / z * d2;

					if (Math.abs(nearD0) <= (this.nearWidth * 0.5) )
						testDistances.push(nearD0);

					if (Math.abs(nearD1) <= (this.nearWidth * 0.5) )
						testDistances.push(nearD1);

					if (Math.abs(nearD2) <= (this.nearWidth * 0.5) )
						testDistances.push(nearD2);
				}
			}
			...
			for (var i=0; i!=testDistances.length; ++i)
			{
				...
				var bestHit = this.scene.rayHitTest(pointNear, rayDir, undefined, undefined, this.hitBackfaces);
			...
			{
				var nearOwner = (this.nearZ > 0.1 || this.nearWidth/2 > 0.1) ? this : undefined;
				var collisionOwner = undefined;//this;

				camera.drawLine(add(nearCenterP, mul(tan, this.nearWidth/2)), add(nearCenterP, mul(tan, -this.nearWidth/2)), this.appearance.GetLineColor(), this.appearance.GetLineWidth(this.selected), [5,5], nearOwner);
				camera.drawLine(add(this.target, mul(tan, this.farWidth/2)), add(this.target, mul(tan, -this.farWidth/2)), this.appearance.GetLineColor(), this.appearance.GetLineWidth(this.selected), [5,5], collisionOwner);

				camera.drawLine(add(nearCenterP, mul(tan, this.nearWidth/2)), add(this.target, mul(tan, this.farWidth/2)), this.appearance.GetLineColor(), this.appearance.GetLineWidth(this.selected), [5,5], collisionOwner);
				camera.drawLine(add(nearCenterP, mul(tan, -this.nearWidth/2)), add(this.target, mul(tan, -this.farWidth/2)), this.appearance.GetLineColor(), this.appearance.GetLineWidth(this.selected), [5,5], collisionOwner);
		...
		if (!this.collisionOutline)
		{
			...
			camera.drawLine(add(nearCenterP, mul(tan, this.nearWidth/2)), add(nearCenterP, mul(tan, -this.nearWidth/2)), this.appearance.GetLineColor(), this.appearance.GetLineWidth(this.selected), [], nearOwner);
		...
		if (this.pixelCount > 0)
		{
			...
			for (var fc=1, fx=-this.farWidth/2+farStep, nx = -this.nearWidth/2+nearStep; fc<this.pixelCount*2; ++fc, fx+=farStep, nx+=nearStep)
			{
				...
				if (fc % 2)
				{
					...
					if (this.showZBuffer)
					{
						var bestHit = this.scene.rayHitTest(pointNear, sub(pointFar,pointNear).unit(), undefined, undefined, this.hitBackfaces);
	...
	this.hitTest = function(a, b, camera)
	{
		...
		if (this.spanCount>1)
		{
			...
			for (var i=1; i!=this.spanCount; ++i)
			{
				...
				if (overlapRectLine(a, b, mad(tan, spanWidth/2, p), mad(tan, -spanWidth/2, p)))	
					return true;
			}
		}
	...
	this.getSnapPoints = function(threshold)
	{
		var tan = transpose(this.dir);
		...
		if (new Vector(this.nearZ, this.nearWidth/2).length() >= threshold)
		{
			points.push({ type: "node", p: mad(tan, +this.nearWidth/2, mad(this.dir, this.nearZ, this.O)) });
			points.push({ type: "node", p: mad(tan, -this.nearWidth/2, mad(this.dir, this.nearZ, this.O)) });
		}
		...
		if (this.pixelCount > 0 && this.nearZ > threshold)
		{
			var farStep = this.farWidth / this.pixelCount / 2;
			var nearStep = this.nearWidth / this.pixelCount / 2;
			...
			for (var fc=0, fx=-this.farWidth/2, nx = -this.nearWidth/2; fc<=this.pixelCount*2; ++fc, fx+=farStep, nx+=nearStep)
			{
				var pointNear = mad(tan, nx, nearCenterP);
				var pointFar = mad(tan, fx, this.target);
